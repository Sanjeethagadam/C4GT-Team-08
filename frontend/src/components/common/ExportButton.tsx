...
export const ExportButton: React.FC<ExportButtonProps> = ({ endpoint, filename, title }) => {
        ...
        if (Array.isArray(data)) {
          ...
          if (data.identity) {
            autoTable(doc, {
              ...
              body: Object.entries(data.identity).map(([k, v]) => [k, String(v || '')]),
              startY: currentY,
              theme: 'grid'
            });
          ...
          if (data.backlogs && data.backlogs.length > 0) {
            ...
            autoTable(doc, {
              ...
              body: data.backlogs.map((b: any) => Object.values(b).map(v => String(v || ''))),
              startY: currentY + 5,
              theme: 'grid'
            });
            currentY = (doc as any).lastAutoTable.finalY + 10;
          ...
          if (data.marks && data.marks.length > 0) {
            ...
            autoTable(doc, {
              ...
              body: data.marks.map((m: any) => Object.values(m).map(v => String(v || ''))),
              startY: currentY + 5,
              theme: 'grid'
            });
            currentY = (doc as any).lastAutoTable.finalY + 10;
          ...
          if (data.results && data.results.length > 0) {
            ...
            autoTable(doc, {
              ...
              body: data.results.map((r: any) => Object.values(r).map(v => String(v || ''))),
              startY: currentY + 5,
              theme: 'grid'
            });
          }
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant="outline" size="sm" disabled={isExporting} className="bg-white">
