...
export const Backlogs = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // We need backlogs by year. We can fetch it by passing year filter
      const [y2, y3, y4] = await Promise.all([
        analyticsService.getBacklogsDistribution({ year: 2 }),
        analyticsService.getBacklogsDistribution({ year: 3 }),
        analyticsService.getBacklogsDistribution({ year: 4 })
      ]);
      const yearDist = [
        { name: 'Year 2', value: y2.activeBacklogSubjects || 0 },
        { name: 'Year 3', value: y3.activeBacklogSubjects || 0 },
        { name: 'Year 4', value: y4.activeBacklogSubjects || 0 }
      ];
