  ...
  useEffect(() => {
    const fetchRisk = async () => {
      try {
        ...
        if (data && data.charts && data.charts.riskDistribution) {
          const low = data.charts.riskDistribution.find((r: any) => r.level === 'LOW')?.count || 0;
          const medium = data.charts.riskDistribution.find((r: any) => r.level === 'MEDIUM')?.count || 0;
          const high = data.charts.riskDistribution.find((r: any) => r.level === 'HIGH')?.count || 0;
        ...
        if (data && data.charts && data.charts.riskByYear) {
          const barData = data.charts.riskByYear.map((item: any) => {
            const low = item.data.find((r: any) => r.level === 'LOW')?.count || 0;
            const medium = item.data.find((r: any) => r.level === 'MEDIUM')?.count || 0;
            const high = item.data.find((r: any) => r.level === 'HIGH')?.count || 0;
            return {
              name: item.year,
              LOW: low,
              MEDIUM: medium,
