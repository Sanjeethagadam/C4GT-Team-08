...
export const Notices = () => {
  ...
  const handlePublish = async (id: string) => {
    try {
      const estimateRes = await apiClient.put(`/notifications/notices/${id}/publish?estimate=true`);
      const count = estimateRes.data?.estimatedCount || 0;
      
      if (!window.confirm(`Target Audience: Selected Branches & Years\nEstimated Student Count: ${count}\n\nAre you sure you want to publish this notice?`)) return;
      
      const publishRes = await apiClient.put(`/notifications/notices/${id}/publish`);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-1 bg-white p-6 rounded-xl shadow-sm border border-slate-200 h-fit">
          ...
          {uploadMessage && (
            <div className={`p-3 rounded-md mb-4 text-sm ${uploadMessage.type === 'success' ? 'bg-emerald-50 text-emerald-800 border border-emerald-200' : 'bg-red-50 text-red-800 border border-red-200'}`}>
              {uploadMessage.text}
            </div>
          )}
          ...
          <div className="space-y-4">
            <div>
              ...
              <Input value={title} onChange={e => setTitle(e.target.value)} placeholder="e.g. Exam Timetable" />
            </div>
            ...
            <div>
              ...
              <Input value={description} onChange={e => setDescription(e.target.value)} placeholder="Brief context..." />
            </div>

            {/* Admin Targeting UI */}
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-2">Target Branches (Empty = All)</label>
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-2">Target Years (Empty = All)</label>
            ...
            <div>
              ...
              <Input id="pdf-upload" type="file" accept="application/pdf" onChange={handleFileChange} className="cursor-pointer file:text-primary file:font-medium file:bg-primary/10 file:border-0 file:mr-4 file:py-1 file:px-3 file:rounded-md hover:file:bg-primary/20" />
            </div>

            <Button onClick={handleUpload} disabled={isUploading || !file || !title} className="w-full">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          {notices.length === 0 && !error ? (
            ...
            notices.map(notice => (
              <div key={notice._id} className="bg-white p-5 rounded-xl shadow-sm border border-slate-200 flex flex-col md:flex-row md:items-center justify-between gap-4 transition-all hover:shadow-md">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <h4 className="font-medium text-slate-800">{notice.title}</h4>
                    <Badge variant="secondary" className="text-xs bg-slate-100 text-slate-700">{notice.noticeType}</Badge>
                    {notice.isPublished ? (
                      <Badge variant="outline" className="bg-emerald-50 text-emerald-700 border-emerald-200 ml-2">Published</Badge>
                    ) : (
                      <Badge variant="outline" className="bg-amber-50 text-amber-700 border-amber-200 ml-2">Draft</Badge>
                    )}
                  </div>
                  {notice.description && <p className="text-sm text-slate-600 mb-2">{notice.description}</p>}
                  <p className="text-xs text-slate-400 mb-2">
                    Targets: {notice.targetBranches?.length ? notice.targetBranches.length + ' Branches' : 'All Branches'} | {notice.targetYears?.length ? 'Years: ' + notice.targetYears.join(', ') : 'All Years'}
                  </p>
                  <p className="text-xs text-slate-400">Uploaded: {new Date(notice.createdAt).toLocaleString()}</p>
                </div>
                
                <div className="flex items-center gap-2 flex-wrap">
                  <Button variant="outline" size="sm" onClick={() => handleViewPdf(notice._id)}>
                    View PDF
                  </Button>
                  
                  {!notice.isPublished ? (
                    <Button variant="default" size="sm" onClick={() => handlePublish(notice._id)} className="bg-emerald-600 hover:bg-emerald-700">
                      <Send className="w-4 h-4 mr-2" />
                      Publish
                    </Button>
                  ) : (
                    <>
                      <Button variant="outline" size="sm" onClick={() => handleViewStats(notice._id)}>
                        Stats
                      </Button>
                      <Button variant="outline" size="sm" onClick={() => handleUnpublish(notice._id)}>
                        Unpublish
                      </Button>
                    </>
                  )}
                  
                  <Button variant="ghost" size="sm" onClick={() => handleDelete(notice._id)} className="text-red-600 hover:bg-red-50 hover:text-red-700">
