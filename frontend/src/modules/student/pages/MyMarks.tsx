    ...
    <div className="space-y-6 pb-12">
      ...
      {isLoading ? (
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Card className="bg-emerald-50 border-emerald-100 shadow-sm">
              <CardHeader className="pb-2">
                <CardTitle className="text-emerald-800 text-lg">Mid-1 Average</CardTitle>
              </CardHeader>
              <CardContent>
                {mid1Avg === 'Not Available' ? (
                  <div className="text-lg font-medium text-emerald-700">Not Available</div>
                ) : (
                  <div className="text-3xl font-bold text-emerald-900">{mid1Avg} <span className="text-sm font-normal text-emerald-700">/ 30</span></div>
                )}
              </CardContent>
            </Card>
            <Card className="bg-blue-50 border-blue-100 shadow-sm">
              <CardHeader className="pb-2">
                <CardTitle className="text-blue-800 text-lg">Mid-2 Average</CardTitle>
              </CardHeader>
              <CardContent>
                {mid2Avg === 'Not Available' ? (
                  <div className="text-lg font-medium text-blue-700">Not Available</div>
                ) : (
                  <div className="text-3xl font-bold text-blue-900">{mid2Avg} <span className="text-sm font-normal text-blue-700">/ 30</span></div>
                )}
              </CardContent>
            </Card>
          </div>
          ...
          {chartData.length > 0 && (
            <Card className="shadow-sm border-slate-200">
              <CardHeader>
                <CardTitle className="text-slate-800">Mid-1 vs Mid-2 Comparison</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="h-[300px] w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={chartData} margin={{ top: 20, right: 30, left: 0, bottom: 5 }}>
                      ...
                      <Bar dataKey="Mid 1" fill="#3b82f6" radius={[4, 4, 0, 0]} maxBarSize={40} />
                      <Bar dataKey="Mid 2" fill="#10b981" radius={[4, 4, 0, 0]} maxBarSize={40} />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
              </CardContent>
        ...
        <div className="bg-white p-12 text-center rounded-xl shadow-sm border border-slate-200">
          ...
          <h3 className="text-lg font-medium text-slate-800 mb-1">No Marks Available</h3>
          <p className="text-slate-500">Your internal marks have not been published yet.</p>
        </div>
      )}
    </div>
  );
