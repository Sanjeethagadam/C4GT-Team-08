...
const getNotificationDetails = (type: string) => {
  switch (type) {
    case 'BACKLOG': return { color: 'bg-rose-100 text-rose-600', path: '/coordinator/backlog-students' };
    case 'REMEDIAL': return { color: 'bg-blue-100 text-blue-600', path: '/coordinator/remedial-classes' };
    case 'GUEST_LECTURE': return { color: 'bg-emerald-100 text-emerald-600', path: '/coordinator/guest-lectures' };
    case 'RESULT_IMPORT': return { color: 'bg-indigo-100 text-indigo-600', path: '/coordinator/dashboard' };
    default: return { color: 'bg-slate-100 text-slate-600', path: '/coordinator/dashboard' };
  }
};
...
export const Notifications = () => {
  ...
  const [activeTab, setActiveTab] = useState<'ALL' | 'REMEDIAL' | 'GUEST_LECTURE' | 'SYSTEM'>('ALL');

  const filteredNotifications = notifications.filter(n => {
    if (activeTab === 'ALL') return true;
    if (activeTab === 'REMEDIAL' && n.notificationType === 'REMEDIAL') return true;
    if (activeTab === 'GUEST_LECTURE' && n.notificationType === 'GUEST_LECTURE') return true;
    if (activeTab === 'SYSTEM' && !['REMEDIAL', 'GUEST_LECTURE'].includes(n.notificationType)) return true;
    return false;
  });
            ...
            return (
              <div 
                ...
                className={`cursor-pointer bg-white rounded-xl border p-4 sm:p-6 transition-all hover:bg-slate-50 ${
                  isRead ? 'border-slate-200 shadow-sm' : 'border-primary/30 shadow-md ring-1 ring-primary/5'
                }`}
              >
