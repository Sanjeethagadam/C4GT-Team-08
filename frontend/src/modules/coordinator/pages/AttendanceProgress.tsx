...
export const AttendanceProgress = () => {
  ...
  const [filter, setFilter] = useState<'All' | 'RemedialClass' | 'GuestLecture'>('All');
  ...
  const handleViewProgress = async (student: any) => {
    ...
    try {
      const response = await apiClient.get(`/academic-support/attendance/history/${student._id}`);
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const response = await apiClient.get(`/academic-support/attendance/progress/all?${params.toString()}`) as any;
  ...
  const histPct = histTotal > 0 ? Math.round((histPresent / histTotal) * 100) : 0;
      ...
      {isLoading ? (
        ...
        <>
          <DataTable 
            data={students}
            columns={[
              { header: 'Roll Number', cell: (row) => row.student?.rollNo || '-' },
              { header: 'Student Name', cell: (row) => row.student?.name || '-' },
              { header: 'Branch', cell: (row) => row.student?.branchId?.code || '-' },
              { header: 'Year', cell: (row) => row.student?.year ? `Year ${row.student.year}` : '-' },
              { header: 'Total Sessions', cell: (row) => row.totalSessions || 0 },
              { header: 'Attendance %', cell: (row) => (
                <span className={`font-medium ${row.attendancePercentage >= 75 ? 'text-emerald-600' : 'text-rose-600'}`}>
                  {row.attendancePercentage || 0}%
                </span>
              ) },
              { header: 'Actions', cell: (row) => (
                <button onClick={() => handleViewProgress(row.student)} className="text-sm text-primary hover:underline font-medium">View Progress</button>
              ) },
            ]}
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-4xl flex flex-col max-h-[90vh]">
            ...
            <div className="px-4 py-3 bg-slate-50 border-b flex justify-between items-center shrink-0">
              <div className="flex gap-2">
                <button onClick={() => setFilter('All')} className={`px-3 py-1 text-sm rounded ${filter === 'All' ? 'bg-primary text-white' : 'bg-white border text-slate-600'}`}>All</button>
                <button onClick={() => setFilter('RemedialClass')} className={`px-3 py-1 text-sm rounded ${filter === 'RemedialClass' ? 'bg-primary text-white' : 'bg-white border text-slate-600'}`}>Remedial</button>
                <button onClick={() => setFilter('GuestLecture')} className={`px-3 py-1 text-sm rounded ${filter === 'GuestLecture' ? 'bg-primary text-white' : 'bg-white border text-slate-600'}`}>Guest Lectures</button>
              </div>
            ...
            <div className="p-4 flex-1 overflow-y-auto">
              {isHistoryLoading ? (
                ...
                <div className="py-8 text-center text-slate-500 italic">No progress records found for this filter.</div>
              ) : (
                    ...
                    <tbody>
                      {filteredHistory.map((record, i) => (
                        <tr key={i} className="border-b last:border-0 hover:bg-slate-50">
                          <td className="p-3">{record.type === 'RemedialClass' ? 'Remedial' : 'Guest Lecture'}</td>
                          <td className="p-3">{record.subject}</td>
                          <td className="p-3">{record.topic}</td>
                          <td className="p-3">{new Date(record.date).toLocaleDateString()}</td>
                          <td className="p-3">{formatTimeAMPM(record.startTime)} - {formatTimeAMPM(record.endTime)}</td>
                          <td className="p-3">{record.venue}</td>
                          <td className="p-3">
                            {record.present ? (
                              <span className="text-green-600 font-medium bg-green-50 px-2 py-0.5 rounded-full">Present</span>
                            ) : (
                              <span className="text-rose-600 font-medium bg-rose-50 px-2 py-0.5 rounded-full">Absent</span>
                            )}
                          </td>
                        </tr>
                      ))}
