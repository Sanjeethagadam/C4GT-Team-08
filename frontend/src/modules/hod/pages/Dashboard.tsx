        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <ChartCard title="Risk Distribution" description="Active backlogs: Low (0-1), Medium (2-4), High (5+)">
              {riskData.length === 0 || (riskData.reduce((a,b)=>a+b.count,0) === 0) ? (
                <div className="flex h-full items-center justify-center text-slate-500">No risk records available.</div>
              ) : (
            ...
            <ChartCard title="Branch Distribution (Students)" description="Number of students per branch">
                ...
                <table className="w-full text-sm text-left">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {kpis?.studentBranches?.map((b) => (
                      <tr key={b.branchCode} className="hover:bg-slate-50 transition-colors">
                  ...
                  {kpis?.studentBranches && kpis.studentBranches.length > 0 && (
                    <tfoot className="bg-slate-50 font-semibold border-t border-slate-200">
                      <tr>
                        ...
                        <td className="px-4 py-3 text-right text-slate-800">{kpis.totalStudents}</td>
                      </tr>
                    </tfoot>
                  )}
                </table>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <ChartCard title="Branch Distribution (Backlogs)" description="Active backlogs per branch">
              <div className="overflow-x-auto">
                <table className="w-full text-sm text-left">
                  <thead className="bg-slate-50 text-slate-600 font-medium border-b border-slate-200">
                    <tr>
                  ...
                  {kpis?.backlogBranches && kpis.backlogBranches.length > 0 && (
                    <tfoot className="bg-slate-50 font-semibold border-t border-slate-200">
                      <tr>
                        <td className="px-4 py-3 text-slate-800">Total</td>
                        <td className="px-4 py-3 text-right text-slate-800">{kpis.totalBacklogs}</td>
                      </tr>
                    </tfoot>
                  )}
                </table>
