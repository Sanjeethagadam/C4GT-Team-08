...
const uploadTimetable = async (req, res) => {
    try {
        ...
        // Clean up temporary file if upload processing failed (if on disk)
...
const downloadTimetableFile = async (req, res) => {
    try {
        ...
        const originalName = timetable.file?.originalName || "timetable.pdf";

        // 1. Primary: Stream directly from MongoDB Atlas Cluster (GridFS)
        if (timetable.file?.gridFsId) {
            try {
                const bucket = timetableService.getTimetableBucket();
                const downloadStream = bucket.openDownloadStream(new mongoose.Types.ObjectId(timetable.file.gridFsId));
                ...
                downloadStream.on("error", (err) => {
                    if (!res.headersSent) {
                        return res.status(404).json({
                            success: false,
                            message: "Timetable document not found in MongoDB cluster: " + err.message
                        });
                    }
                });
                ...
                console.warn("GridFS stream initialization failed, trying local fallback:", gridFsErr.message);
            }
        }

        // 2. Backward compatibility: Fallback to reading legacy file from server disk if existing
        ...
        if (!filePath || !fs.existsSync(filePath)) {
            return res.status(404).json({
                success: false,
                message: "Timetable document file not found in MongoDB cluster or on server"
            });
        }
