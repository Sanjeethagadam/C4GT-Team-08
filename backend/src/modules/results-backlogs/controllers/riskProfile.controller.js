const riskProfileService = require('../services/riskProfile.service');
const { sendSuccess, sendError } = require('../../../utils/response');

exports.create = async (req, res) => {
  try {
    const data = await riskProfileService.create(req.body);
...
exports.getAll = async (req, res) => {
  try {
    ...
    const data = await RiskProfile.find(query);
...
exports.getById = async (req, res) => {
  try {
    const data = await riskProfileService.findById(req.params.id);
...
exports.update = async (req, res) => {
  try {
    const data = await riskProfileService.update(req.params.id, req.body);
    if (!data) return sendError(res, 404, 'Not found');
    return sendSuccess(res, 200, data);
  } catch (err) {
    return sendError(res, 400, err.message);
...
exports.remove = async (req, res) => {
  try {
    const data = await riskProfileService.remove(req.params.id);
    if (!data) return sendError(res, 404, 'Not found');
    return sendSuccess(res, 200, data);
  } catch (err) {
    return sendError(res, 400, err.message);
