...
async function testRole(username, roleDesc, checkFn) {
  ...
  const res = await request(app)
    .get('/api/v1/results-backlogs/results')
  ...
  const distinctStudentIds = [...new Set(results.map(r => r.studentId.toString()))];
  ...
  const checkResult = checkFn({ results, distinctStudentIds, campuses, years, user, students });
...
async function runTests() {
  ...
  console.log('--- RESULTS SCOPE TEST ---');
