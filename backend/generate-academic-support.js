...
const entities = ['RemedialClass', 'RemedialStudent', 'GuestLecture', 'Notification'];
const moduleName = 'academic-support';
...
router.post('/', auth, authorizeRoles('ADMIN', 'COORDINATOR'), ${name}Validation, ${name}Controller.create);
router.get('/', auth, ${name}Controller.getAll);
router.get('/:id', auth, ${name}Controller.getById);
router.put('/:id', auth, authorizeRoles('ADMIN', 'COORDINATOR'), ${name}Validation, ${name}Controller.update);
router.delete('/:id', auth, authorizeRoles('ADMIN', 'COORDINATOR'), ${name}Controller.remove);
