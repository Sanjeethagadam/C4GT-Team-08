...
async function testRole(username, roleDesc, checkFn) {
  ...
  const res = await request(app)
    .get('/api/v1/academic-support/remedial-students')
  ...
  const distinctStudentIds = [...new Set(records.map(r => r.studentId.toString()))];
  ...
  const checkResult = checkFn({ records, distinctStudentIds, campuses, years, user, students });
...
async function runTests() {
  ...
  console.log('--- REMEDIAL STUDENTS SCOPE TEST ---');

  await testRole('hod_kiet_group_year1', 'KIET/KIET+ HOD', (data) => {
    // Only assigned year (1), KIET + KIET+
    // But since `seed.js` only seeds for the FIRST branch of KIET, the total records will be small, but campuses should only be KIET or KIET+
  ...
  await testRole('hod_kietw_year1', 'KIET-W HOD', (data) => {
    // KIET-W HOD should see NO KIET records. So campuses must only be KIET-W, and since there are no KIET-W records in seed, it should be empty.
