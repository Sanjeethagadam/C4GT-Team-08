...
async function testRole(username, roleDesc, checkFn) {
  ...
  const marks = res.body.data || [];
  const distinctStudentIds = [...new Set(marks.map(m => m.studentId.toString()))];
  
  // fetch students to verify campus and year
  ...
  const checkResult = checkFn({ marks, distinctStudentIds, campuses, years, user, students });
  const statusStr = checkResult ? 'PASS' : 'FAIL';
  
  console.log(`Role: ${roleDesc} (${username}) -> ${statusStr}`);
  console.log(`  Total marks: ${marks.length}`);
  console.log(`  Distinct studentIds: ${distinctStudentIds.length}`);
...
async function runTests() {
  ...
  console.log('--- MARKS SCOPE TEST ---');

  // 1. KIET-W HOD: hod_kietw_year1
  await testRole('hod_kietw_year1', 'KIET-W HOD', (data) => {
    return data.campuses.length === 1 && data.campuses[0] === 'KIET-W' && data.years.length === 1 && data.years[0] === 1;
  });

  // 2. KIET/KIET+ HOD: hod_kiet_group_year1
  await testRole('hod_kiet_group_year1', 'KIET/KIET+ HOD', (data) => {
    return data.campuses.every(c => c === 'KIET' || c === 'KIET+') && data.years.length === 1 && data.years[0] === 1;
  });

  // 3. CTPO: ctpo_kiet
  await testRole('ctpo_kiet', 'CTPO', (data) => {
    // All students must belong to the single section assigned to CTPO
    const sectionIds = [...new Set(data.students.map(s => s.sectionId.toString()))];
    return sectionIds.length === 1 && sectionIds[0] === data.user.scope.sectionId.toString();
  });
