...
const entities = ['Examination', 'Timetable', 'Mark'];
...
`;
  ...
  // Validator
  const validatorCode = `const { validate } = require('../../../../middlewares/validate');
const { body } = require('express-validator');

exports.${name}Validation = validate([
  // add validation logic later
]);
`;
  fs.writeFileSync(path.join(__dirname, `src/modules/${moduleName}/validators/${name}.validator.js`), validatorCode);
});
